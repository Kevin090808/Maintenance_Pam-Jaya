...
$stats_query = "SELECT 
                ...
                AVG(jumlah_part) as avg_temp, 
                SUM(CASE WHEN good = '✔' THEN 1 ELSE 0 END) as good_units,
                SUM(CASE WHEN not_good = '✔' THEN 1 ELSE 0 END) as not_good_units
                FROM inspeksi_valve";
...
if (!empty($selected_date)) {
    ...
    if ($result_date->num_rows > 0) {
        ...
        // Query for valve inspection data with proper joins
        $sql_valve = "SELECT iv.*, d.tanggal, nw.nomer as no_wo, p.nama as plant, 
                      l.nama as lokasi, f.nama as frequensi
                      FROM inspeksi_valve iv
                      JOIN date d ON iv.id_date = d.id_date
                      JOIN no_wo nw ON iv.id_date = nw.id_date
                      JOIN plant p ON iv.id_date = p.id_date
                      JOIN lokasi l ON iv.id_date = l.id_date
                      JOIN frequensi f ON iv.id_date = f.id_date
                      WHERE d.tanggal = ?";
        $stmt_valve = $conn->prepare($sql_valve);
        $stmt_valve->bind_param('s', $selected_date);
        $stmt_valve->execute();
        $result_valve = $stmt_valve->get_result();

        // Query for maintenance data - ADDED HERE
    ...
    $sql_valve = "SELECT iv.*, d.tanggal, nw.nomer as no_wo, p.nama as plant, 
                  ...
                  JOIN lokasi l ON iv.id_date = l.id_date
                  JOIN frequensi f ON iv.id_date = f.id_date";
    $result_valve = $conn->query($sql_valve);
    $has_data = $result_valve->num_rows > 0;
...
<body>
    ...
    <div class="container">
        ...
        <div class="row mb-4">
            <div class="col-md-6">
                <a href="inspeksi_valve.php" class="btn btn-secondary me-2">
    ...
    <div class="container">
        ...
        <div class="page-header">
            ...
            <p>Laporan detail hasil inspeksi valve dan data maintenance.</p>
        </div>
        ...
        <div class="row summary-cards mb-4">
            ...
            <div class="col-md-3 col-sm-6">
                <div class="summary-card danger">
                    ...
                    <p>Kondisi Not Good</p>
                </div>
            </div>
            <div class="col-md-3 col-sm-6">
                <div class="summary-card warning">
                    <i class="far fa-calendar-check"></i>
                    <h3><?php echo !empty($selected_date) ? '1' : 'All'; ?></h3>
        ...
        <div class="card mb-4">
            ...
            <!-- Valve Inspection Details Section -->
            <table class="table table-hover table-striped mt-4">
            <thead>
                    <tr>
                        <th>No</th>
                        <th>equipment</th>
                        <th colspan="7" class="text-center">valve</th>
                ...
                <tbody>
                    <?php 
                    // Reset pointer and loop again for inspection details
                    if ($has_data && $result_valve->num_rows > 0) {
                        $result_valve->data_seek(0);
                        while ($row = $result_valve->fetch_assoc()): 
                    ?>
                        <tr>
                            <td><?= htmlspecialchars($row['id']) ?></td>
                            <td><?= htmlspecialchars($row['equipment']) ?></td>
                            <td><?= htmlspecialchars($row['butterfly']) ?></td>
                            <td><?= htmlspecialchars($row['gate']) ?></td>
                            <td><?= htmlspecialchars($row['ball']) ?></td>
                            <td><?= htmlspecialchars($row['globe']) ?></td>
                            <td><?= htmlspecialchars($row['membran']) ?></td>
                            <td><?= htmlspecialchars($row['foot_valve']) ?></td>
                            <td><?= htmlspecialchars($row['swing_check']) ?></td>
                            <td>
                                <?php if ($row['good'] == '1'): ?>
                            ...
                            <td>
                                <?php if ($row['not_good'] == '1'): ?>
                                    <span class="badge bg-danger">✔</span>
                                <?php else: ?>
                                    <span class="badge bg-secondary">✖</span>
                                <?php endif; ?>
                            </td>
                            <td><?= htmlspecialchars($row['perawatan_part']) ?></td>
                            <td><?= htmlspecialchars($row['penggantian_part']) ?></td>
                            <td><?= htmlspecialchars($row['jumlah_part']) ?></td>
                            <td><?= htmlspecialchars($row['Remaks']) ?></td>
                        </tr>
                    <?php 
                        endwhile;
                    } else {
                    ?>
                        <tr>
                            <td colspan="15" class="text-center">No valve inspection data found</td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="view_valve.php" method="GET">
                        <div class="mb-3">
    ...
    <script>
        ...
        function exportToExcel() {
            ...
            var template = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Data Inspeksi Valve</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--><meta http-equiv="content-type" content="text/plain; charset=UTF-8"/></head><body>';
            ...
            link.download = 'Data_Inspeksi_Valve_<?php echo !empty($selected_date) ? $selected_date : date('Y-m-d'); ?>.xls';
