...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $nama_Part = isset($_POST['nama_Part']) ? mysqli_real_escape_string($conn, $_POST['nama_Part']) : '';
    ...
    $detail_pekerjaan = isset($_POST['detail_pekerjaan']) ? mysqli_real_escape_string($conn, $_POST['detail_pekerjaan']) : '';
    ...
    if (empty($no_wo) || empty($tanggal) || empty($plant) || empty($lokasi) || 
        empty($bulan) || empty($nama) || empty($equipment) || empty($deskripsi) || 
        empty($nama_Part) || empty($penggantian_part) || empty($jumlah_part) || empty($detail_pekerjaan) || empty($status)) {
        ...
        try {
            ...
            // Insert data ke tabel valve_tuneup
            $sql_inspeksi_ac = "INSERT INTO valve_tuneup (equipment, deskripsi, nama_Part, penggantian_part, jumlah_part, detail_pekerjaan, status, id_date) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt_valve_tuneup = $conn->prepare($sql_inspeksi_ac);
            if (!$stmt_valve_tuneup) {
                throw new Exception("Prepare failed: " . $conn->error);
            }
            $stmt_valve_tuneup->bind_param("sssssssi", $equipment, $deskripsi, $nama_Part, $penggantian_part, $jumlah_part, $detail_pekerjaan, $status, $id_date);
            if (!$stmt_valve_tuneup->execute()) {
                throw new Exception("Execute failed: " . $stmt_valve_tuneup->error);
            }
            ...
            echo "<script>
                    ...
                    window.location.href = 'valve_tuneup.php';
                  </script>";
        } catch (Exception $e) {
            ...
            if (isset($stmt_valve_tuneup)) $stmt_valve_tuneup->close();
        }
    }
}
...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
            ...
            <tr>
                ...
                <td>
                    <div class="frequency-container">
                        ...
                        <input list="nama" name="nama" placeholder="Pilih Tahun" >
                        <datalist id="nama">
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    <tr>
                        <td><input type="text" name="equipment" required></td>
                        <td><input type="text" name="deskripsi" required></td>
                        <td><input type="text" name="nama_Part" required></td>
                        <td><input type="text" name="penggantian_part" required></td>
                        <td><input type="text" name="jumlah_part" required></td>
                        <td><input type="text" name="detail_pekerjaan" required></td>
                        <td>
                            <select name="status" required>
                                <option value="">Pilih Status</option>
                                <option value="O">O (Berfungsi)</option>
                                <option value="X">X (Tidak Berfungsi)</option>
                            </select>
                        </td>
                    </tr>
                </tbody>
    ...
    <div class="button-container">
        <button onclick="window.location.href='valve-tuneup.php'">Kembali</button>
    </div>
<footer>
