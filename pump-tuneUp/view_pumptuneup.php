...
try {
    ...
    if (isset($_POST['delete_id']) && is_numeric($_POST['delete_id'])) {
        ...
        $check_sql = "SELECT id_date FROM pump_tuneup WHERE id_date = ?";
        ...
        if ($check_result->num_rows > 0) {
            ...
            try {
                ...
                $queries = [
                    ["DELETE FROM pump_tuneup WHERE id = ?", "i", [$delete_id]],
    ...
    if (!empty($_GET['search'])) {
        ...
        $like = "%{$search}%";
        $where_conditions[] = "(nw.nomer LIKE ? OR pt.equipment LIKE ? OR l.nama LIKE ?)";
    ...
    $count_sql = "
        ...
        FROM pump_tuneup pt
        JOIN date d ON pt.id_date = d.id_date
        JOIN no_wo nw ON pt.id_date = nw.id_date
        JOIN plant p ON pt.id_date = p.id_date
        JOIN lokasi l ON pt.id_date = l.id_date
        JOIN frequensi f ON pt.id_date = f.id_date
        $where_clause
    ";
    ...
    $sql = "
        ...
        JOIN frequensi f ON pt.id_date = f.id_date
        $where_clause
        ORDER BY d.tanggal DESC, pt.id DESC
        LIMIT ? OFFSET ?
    ";
    ...
    error_log("Error pada pump_tuneup.php: " . $e->getMessage());
} finally {
...
<head>
    ...
    <title>View Data Inspeksi Pump Tuneup</title>
...
<body>
    <div class="container">
        <div class="page-container">
            <div class="header">
                ...
                <h1>Data Inspeksi Pump Tuneup</h1>
                ...
                <div class="controls-section">
                    <form method="GET" action="">
                        ...
                        <div class="action-buttons">
                            ...
                            <a href="input_pumptuneup.php" class="btn btn-success">
    ...
    <!-- First Table for Header Information -->
    <table class="table">
        <thead>
            <tr>
                <th>No</th>
        ...
        <tbody>
            ...
            ?>
            <tr class="header-row" data-id="<?php echo $row['id']; ?>">
                <td><?php echo $no++; ?></td>
                ...
                <td>
                    <div class="action-buttons">
                        <a href="pump_tuneup_edit.php?id=<?php echo isset($row['id']) ? $row['id'] : ''; ?>" class="btn btn-primary btn-sm" title="Edit">
                            <i class="fas fa-edit"></i>
                        </a>
                        <button class="btn btn-danger btn-sm delete-btn" data-id="<?php echo isset($row['id']) ? $row['id'] : ''; ?>" title="Delete">
                            <i class="fas fa-trash"></i>
                        </button>
                        <button class="btn btn-info btn-sm toggle-details" data-id="<?php echo $row['id']; ?>" title="Show Details">
                            <i class="fas fa-chevron-down"></i>
                        </button>
                    </div>
                </td>
            </tr>
            <!-- Second Table for Equipment Details (hidden by default) -->
            <tr class="details-row" id="details-<?php echo $row['id']; ?>" style="display: none;">
                <td colspan="7">
                    <table class="details-table">
                        ...
                        <tbody>
                            <tr>
                                <td class="wrap-text"><?php echo isset($row['equipment']) ? htmlspecialchars($row['equipment']) : '-'; ?></td>
                                <td><?php echo isset($row['seal_coupling']) ? htmlspecialchars($row['seal_coupling']) : '-'; ?></td>
                                <td><?php echo isset($row['shaft']) ? htmlspecialchars($row['shaft']) : '-'; ?></td>
                                <td><?php echo isset($row['bolt_mounting']) ? htmlspecialchars($row['bolt_mounting']) : '-'; ?></td>
                                <td><?php echo isset($row['balancing']) ? htmlspecialchars($row['balancing']) : '-'; ?></td>
                                <td>
                                    <?php if (isset($row['good']) && $row['good']): ?>
                                    <span class="status-badge badge-good">Good</span>
                                    <?php endif; ?>
                                    <?php if (isset($row['not_good']) && $row['not_good']): ?>
                                    <span class="status-badge badge-not-good">Not Good</span>
                                    <?php endif; ?>
                                    <?php if ((!isset($row['good']) || !$row['good']) && (!isset($row['not_good']) || !$row['not_good'])): ?>
                                    <span>-</span>
                                    <?php endif; ?>
                                </td>
                                <td class="wrap-text"><?php echo isset($row['perawatan_part']) ? htmlspecialchars($row['perawatan_part']) : '-'; ?></td>
                                <td class="wrap-text"><?php echo isset($row['penggantian_part']) ? htmlspecialchars($row['penggantian_part']) : '-'; ?></td>
                                <td><?php echo isset($row['jumlah_part']) ? htmlspecialchars($row['jumlah_part']) : '-'; ?></td>
                                <td class="wrap-text"><?php echo isset($row['remaks']) ? htmlspecialchars($row['remaks']) : (isset($row['remarks']) ? htmlspecialchars($row['remarks']) : '-'); ?></td>
                            </tr>
                        </tbody>
                    </table>
                </td>
