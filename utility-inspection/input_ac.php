...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $no_ac = isset($_POST['no_ac']) ? mysqli_real_escape_string($conn, $_POST['no_ac']) : '';
    $pk = isset($_POST['pk']) ? mysqli_real_escape_string($conn, $_POST['pk']) : '';
    $merek = isset($_POST['merek']) ? mysqli_real_escape_string($conn, $_POST['merek']) : '';
    $tipe = isset($_POST['tipe']) ? mysqli_real_escape_string($conn, $_POST['tipe']) : '';
    $freon = isset($_POST['freon']) ? mysqli_real_escape_string($conn, $_POST['freon']) : '';
    $suhu_ruangan = isset($_POST['suhu_ruangan']) ? mysqli_real_escape_string($conn, $_POST['suhu_ruangan']) : '';
    $fungsi = isset($_POST['fungsi']) ? mysqli_real_escape_string($conn, $_POST['fungsi']) : '';
    ...
    if (empty($no_wo) || empty($tanggal) || empty($plant) || empty($lokasi) || 
        empty($bulan) || empty($nama) || empty($no_ac) || empty($pk) || 
        empty($merek) || empty($tipe) || empty($freon) || empty($suhu_ruangan) || empty($fungsi)) {
        ...
        try {
            ...
            // Insert data ke tabel inspeksi_ac
            $sql_inspeksi_ac = "INSERT INTO inspeksi_ac (no_ac, pk, merek, tipe, freon, suhu_ruangan, fungsi, catatan, id_date) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt_inspeksi_ac = $conn->prepare($sql_inspeksi_ac);
            if (!$stmt_inspeksi_ac) {
                throw new Exception("Prepare failed: " . $conn->error);
            }
            $stmt_inspeksi_ac->bind_param("ssssssssi", $no_ac, $pk, $merek, $tipe, $freon, $suhu_ruangan, $fungsi, $catatan, $id_date);
            if (!$stmt_inspeksi_ac->execute()) {
                throw new Exception("Execute failed: " . $stmt_inspeksi_ac->error);
            }
            ...
            echo "<script>
                    ...
                    window.location.href = 'inspeksi-ac.php';
                  </script>";
        } catch (Exception $e) {
            ...
            if (isset($stmt_inspeksi_ac)) $stmt_inspeksi_ac->close();
        }
    }
}
...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    <tr>
                        <td><input type="text" name="no_ac" required></td>
                        <td><input type="text" name="pk" required></td>
                        <td><input type="text" name="merek" required></td>
                        <td><input type="text" name="tipe" required></td>
                        <td><input type="text" name="freon" required></td>
                        <td><input type="text" name="suhu_ruangan" required></td>
                        <td>
    ...
    <div class="button-container">
        <button onclick="window.location.href='inspeksi-ac.php'">Kembali</button>
    </div>
<footer>
